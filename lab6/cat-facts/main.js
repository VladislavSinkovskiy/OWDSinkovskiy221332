...
window.onload = function () {
    ...
    document.querySelector('.search-form').onsubmit = searchFormHandler;// Добавляем обработчик для формы поиска
    // Добавляем обработчик для ввода в текстовое поле
    document.querySelector('.search-input').oninput = function (event) {
        clearTimeout(autocompleteTimer);  // Очищаем предыдущий таймер задержки
    ...
    document.querySelector('.search-input').oninput = function () {
        showAutocompleteList();// Вызов функции "autocomplete" с текущим значением в поле ввода
    ...
    document.querySelector('.autocomplete-list').onclick = function (event) {
        if (event.target.tagName === 'LI') {
            ...
            document.querySelector('.autocomplete-list').innerHTML = '';// Очистка списка автодополнения
        }
    };
